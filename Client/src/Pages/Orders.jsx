...
const Orders = () => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'Delivered':
        return 'bg-green-500/20 text-yellow-400'
        break;
        ...
        <div className="glass-card p-4 mb-8">
          <div className="flex items-center space-x-4 flex-wrap">
            <div className="flex items-center space-x-2">
              <Filter className="w-5 h-5 text-primary cursor-pointer"/>
            ...
            <div className="flex flex-wrap gap-2">
              {
                statusArray.map((status) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg font-medium transition-all capitalize ${statusFilter === status ? 'gradient-primary text-primary-foreground' : 'glass-card hover:glow-on-hover text-foreground '} cursor-pointer`}
                  >
                    {status}
                  </button>
                ))
        ...
        {/* Orders List */}
        {
          filterOrders.length === 0 ? (
            <div className="text-center glass-panel max-w-md mx-auto ">
              ...
              <p className="text-muted-foreground">{statusFilter === 'All' ? 'You have not placed any orders yet.' : `No orders with status ${statusFilter} found.`}</p>
            </div>
          ) : (
                ...
                filterOrders.map((order) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 space-y-4 md:space-y-0">
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="text-right ">
                          ...
                          <p className="text-xl text-primary font-bold">${order.total_price}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3 mt-6 pt-6 border-t border-[hsla(var(--glass-border))]">
                      <button className="px-4 py-2 glass-card hover:glow-on-hover animate-smooth text-sm cursor-pointer">
                        View Details
                      </button>
                      <button className="px-4 py-2 glass-card hover:glow-on-hover animate-smooth text-sm cursor-pointer">
                        Track Order
